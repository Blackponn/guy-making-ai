...
import {Skintones} from '../components/skintone';
...
export default function Home() {
  return (
    <>
      ...
      <h1 className="text-center text-[32px] font-bold text-[#000000] pt-[72px]">{"Skintone"}</h1>
      <h2 className="text-center text-[16px] font-regular text-[#636363]">{"5/8"}</h2>
      <div className="mt-[64px] px-[64px] flex flex-wrap justify-center gap-[16px] mx-auto" >
        {Skintones.map(skintone => (
          <ColorchangeButton 
            key={skintone.id}
            Name={skintone.skintone}
          />
        ))}
      </div>
      <nav className="text-center shadow-lg w-[128px] my-[64px] text-[32px] text-white bg-[#E35772] rounded-[50pt] border border-[#CB435D] mx-auto">
        <Link href="/facialfeature">{"Next"}</Link>
      </nav>
      </>
